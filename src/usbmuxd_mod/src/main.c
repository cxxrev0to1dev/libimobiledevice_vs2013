...
static int parse_customize_opts(int argc, char *argv[]){
	int result1,result2 = -1;
	for (int i = 0; i < argc; ++i){
		if (!strcmp(argv[i],"-sp")){
			strcpy(socket_path,argv[i+1]);
			++i;
			result1 = 1;
		}
		else if (!strcmp(argv[i],"-spid")){
			strcpy(lockfile,argv[i+1]);
			++i;
			result2 = 1;
		}
	}
...
static void parse_opts(int argc, char **argv)
{
...
#ifdef HAVE_SYSTEMD
	const char* opts_spec = "hfvVuU:xXsnzpid";
#elif HAVE_UDEV
	const char* opts_spec = "hfvVuU:xXnzpid";
#else
	const char* opts_spec = "hfvVU:xXnzpid";
#endif
	...
	while (1) {
		...
		case 'X':
			...
			break;
		default:
			if(parse_customize_opts(argc,argv)){
				continue;
			}
			usage();
			exit(2);
...
//sudo ./usbmuxd --user usbmuxdxx --systemd -sp /var/run/usbmuxdxx -spid /var/run/usbmuxdxx.pid
int usbmuxd_main(int argc, char *argv[])
{
	int listenfd;
	int res = 0;
	int lfd;
	struct flock lock;
	char pids[10];
	parse_customize_opts(argc,argv);
...
#ifndef HAVE_LIBUSBMUXD
int main(int argc, char *argv[]){
	return usbmuxd_main(argc,argv);
}
#endif